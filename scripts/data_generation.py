...
if __name__ == "__main__":
    ...
    bounds = (-2.5, 2.5)  # Area bounds
    min_distance = 1.0  # Minimum distance between agents and goals
    max_velocity = 0.5  # Maximum magnitude of initial velocity

    for scenario_id in range(200):
        # Generate agents and goals
        agents, goals = generate_agents_and_goals(N, bounds, min_distance, max_velocity)

        # Save to CSV
        save_to_csv(agents, goals, filename=f"/home/tq877/Tianyu/player_selection/MCP/data_closer/scenario_{scenario_id}.csv")

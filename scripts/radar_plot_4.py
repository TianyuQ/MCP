...
options = ["threshold", "ranking2", "ranking3"]
test_list = [161, 162, 163, 166, 167, 168, 171, 173, 175, 177, 178, 179, 180, 181, 182, 184, 186, 187, 188, 189, 191]
# option = "threshold"
# option = "ranking2"
# opttion = "ranking3"
for option in options:
    ...
    elif option == "ranking2":
        selected_methods = {
            "Neural Network Rank [2]",
            "Neural Network Partial Rank [2]",
            "Nearest Neighbor [2]",
            "Jacobian [2]",
            "Hessian [2]",
            "Cost Evolution [2]",
    ...
    result_dir = "/home/tq877/Tianyu/player_selection/MCP/data_test_4 _30"
    metrics_dict = {}
    ...
    for metric in metric_names:
        ...
        print(f"{metric} min: {metric_ticks[metric]['min']}, mean: {metric_ticks[metric]['mean']}, max: {metric_ticks[metric]['max']}")

    metric_ticks["Smoothness"]["min"] = 0.02
    metric_ticks["Smoothness"]["max"] = 0.08
    metric_ticks["Length"]["min"] = 2
    metric_ticks["Length"]["max"] = 2.6
    metric_ticks["Safety"]["min"] = 0.5
    metric_ticks["Safety"]["max"] = 1.5
    metric_ticks["Mask Sum"]["min"] = 1
    metric_ticks["Mask Sum"]["max"] = 4
    ...
    plt.tight_layout()
    plt.savefig(f"radar_plot_{option}_4.pdf", dpi=1000)
    print(f"Radar plot saved as 'radar_plot_{option}_4.pdf'.")

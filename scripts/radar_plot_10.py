...
for option in options:
    ...
    result_dir = "/home/tq877/Tianyu/player_selection/MCP/data_test_10 _30"
    metrics_dict = {}
    ...
    for mode in modes_with_params:
        for param in modes_with_params[mode]:
            ...
            for scenario_id in [40, 41, 43, 44, 45, 46]:
    ...
    metric_ticks["Smoothness"]["min"] = 0.01
    metric_ticks["Smoothness"]["max"] = 0.04
    metric_ticks["Length"]["min"] = 5.5
    metric_ticks["Length"]["max"] = 6.5
    metric_ticks["Safety"]["min"] = 0.5
    metric_ticks["Safety"]["max"] = 2.0
    ...
    plt.tight_layout()
    plt.savefig(f"radar_plot_{option}_10.pdf", dpi=1000)
    print(f"Radar plot saved as 'radar_plot_{option}_10.pdf'.")
    # plt.show()
